...
public class DrillSession extends Session {
	...
	public void start() {
		...
		switch (id) {
			...
			case "mace_chase", "spear_smash", "ely_full" -> dummy(DummyBrain.Style.WANDER, false);
	...
	private String shortGoal() {
		return switch (id) {
			...
			case "ely_climb" -> "Wings on, rocket to the marker";
	...
	protected void live() {
		...
		if (chain > 0 && ticks - lastSmashTick > 80) {
			chain = 0;
			say("Chain dropped", "bad");
		}
		...
		if (drill.seconds > 0) {
			...
			String shown = id.equals("ely_climb") ? (score > 0 ? fmt(score) + "s best" : "no time yet") : fmt(score) + " " + drill.unit;
	...
	private void tickClimb() {
		double goal = arena.y + 60;
		if (ticks % 4 == 0) {
			Vec3d c = arena.playerSpawn();
			for (int i = 0; i < 12; i++) {
				double a = i / 12.0 * Math.PI * 2;
				world.spawnParticles(player, ParticleTypes.END_ROD, true, true, c.x + Math.cos(a) * 6, goal, c.z + Math.sin(a) * 6, 1, 0, 0, 0, 0);
			}
		}
		if (!climbing) {
			if (!player.isOnGround() && player.getEntityPos().distanceTo(arena.playerSpawn()) < 6) {
				climbing = true;
				climbStart = ticks;
				airTicks = 0;
			}
		...
		if (player.getY() >= goal) {
			...
			hits++;
			say("Reached the marker in " + fmt(seconds) + "s", "good");
			ding();
			nextClimb();
		} else if ((player.isOnGround() && airTicks > 10) || ticks - climbStart > 300) {
			say("Did not make the marker", "bad");
			nextClimb();
		}
	}
